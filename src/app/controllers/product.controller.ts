    ...
    static getProducts(req: Request, res: Response, next: NextFunction) {
        Product.find({}, (err: Errback, result: any) => {
            if(err) {
                ...
                res.status(200).json(
                    { status: 'success', message: 'Products found', data: result }
                );
            }
        });
    }
            ...
            if(err) {
                ...
                Product.find({ category: category }, (err: Errback, result: any) => {
                    if(err) {
                        ...
                        res.status(200).json(
                            { status: 'success', message: 'Product found', data: result, count: productCount }
                        );
                    }
                });
            } 
        ...
        Product.findByIdAndUpdate(req.body._id,
            {
                ...
                if(err) {
                    ...
                    res.status(200).json(
                        { status: 'success', message: 'Product Updated', data: result }
                    );
                }
            }
        );
